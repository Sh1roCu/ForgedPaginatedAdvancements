...
public class PaginatedAdvancementTab extends AdvancementTabGui {
    ...
    private @Nullable AdvancementEntryGui hoveredWidget;
    ...
    public @Nonnull ITextComponent getTitle() {
        return this.title;
    }
    ...
    public void drawBackground(MatrixStack matrixStack, int x, int y, boolean selected, int atIndex) {
        PaginatedAdvancementTabType.drawBackground(matrixStack, this, x, y, selected, atIndex);
    }
    ...
    public void drawPinnedBackground(MatrixStack matrixStack, int x, int y, boolean selected, int maxPinnedIndex) {
        if (this.pinnedIndex <= maxPinnedIndex) {
            PinnedAdvancementTabType.drawBackground(matrixStack, this, x, y, selected, this.pinnedIndex);
        }
    }
    ...
    public void render(MatrixStack matrixStack, int startX, int startY, int endX, int endY) {
        ...
        fill(matrixStack, advancementTreeWindowWidth, advancementTreeWindowHeight, 0, 0, -16777216);
        RenderSystem.depthFunc(515);
        ResourceLocation identifier = this.display.getBackground();
        // RenderSystem.setShader(GameRenderer::getPositionTexShader);
        //RenderSystem.setShaderTexture(0, Objects.requireNonNullElse(identifier, TextureManager.MISSING_IDENTIFIER));
        Minecraft.getInstance().getTextureManager().bind(identifier != null ? identifier : TextureManager.INTENTIONAL_MISSING_TEXTURE);
        ...
        for (int m = -1; m < textureCountX; ++m) {
            for (int n = -1; n < textureCountY; ++n) {
                blit(matrixStack, k + 16 * m, l + 16 * n, 0.0F, 0.0F, 16, 16, 16, 16);
            }
        }

        this.rootWidget.drawConnectivity(matrixStack, i, j, true);
        this.rootWidget.drawConnectivity(matrixStack, i, j, false);
        this.rootWidget.draw(matrixStack, i, j);
        RenderSystem.depthFunc(518);
        matrixStack.translate(0.0D, 0.0D, -950.0D);
        RenderSystem.colorMask(false, false, false, false);
        fill(matrixStack, 4680, 2260, -4680, -2260, -16777216);
    ...
    public void drawWidgetTooltip(MatrixStack matrixStack, int mouseX, int mouseY, int startX, int startY, int endXWindow, int endY) {
        matrixStack.pushPose();
        RenderSystem.enableDepthTest();
        ...
        if (PaginatedAdvancementsClient.CONFIG.FadeOutBackgroundOnAdvancementHover) {
            fill(matrixStack, 0, 0, endXWindow - startX - 18, endY - startY - 26, MathHelper.floor(this.alpha * 255.0D) << 24);
        }
        ...
        if (mouseX > 0 && mouseX < endXWindow - startX - 10 && mouseY > 0 && mouseY < endY - startY) {
            for (AdvancementEntryGui advancementWidget : this.widgets.values()) {
                if (advancementWidget.isMouseOver(i, j, mouseX, mouseY)) {
                    hoversWidget = true;
                    advancementWidget.drawHover(matrixStack, i, j, this.alpha, startX, startY);
    ...
    public void drawDebugInfo(MatrixStack matrixStack, int startX, int endX, int endY) {
        if (this.hoveredWidget != null) {
            ...
            drawDebugFrame(matrixStack, startX, startY, endX, endY, advancementWidgetAccessor, advancementProgressAccessor);
            drawDebugText(matrixStack, startX + 5, startY + 5, endX, endY, advancementWidgetAccessor, advancementProgressAccessor);
        }
    }

    private void drawDebugText(MatrixStack matrixStack, int startX, int startY, int endX, int endY, AdvancementWidgetAccessor widgetAccessor, AdvancementProgressAccessor progressAccessor) {
        AdvancementProgress progress = widgetAccessor.getProgress();
        Iterable<String> obtainedCriteria = progress.getCompletedCriteria();

        String[][] requirements = progressAccessor.getRequirements();

        ITextComponent idText = new StringTextComponent("ID: " + widgetAccessor.getAdvancement().getId().toString() + " ").append(new TranslationTextComponent("text.paginated_advancements.copy_to_clipboard"));
        this.client.font.drawShadow(matrixStack, idText, startX, startY, 0xFFFFFF);

        for (String[] requirementGroup : requirements) {
            startY += 10;
            IFormattableTextComponent text = new TranslationTextComponent("text.paginated_advancements.group").withStyle(TextFormatting.DARK_RED);
            boolean anyDone = false;
            for (String requirementString : requirementGroup) {
                TextFormatting formatting = TextFormatting.DARK_RED;
                ...
                text.append(new StringTextComponent(requirementString + " ").withStyle(formatting));
            }

            if (anyDone) {
                text.withStyle(TextFormatting.DARK_GREEN);
            }

            this.client.font.draw(matrixStack, text, startX, startY, 0x00ff00);
        }
    }

    public void drawDebugFrame(MatrixStack matrixStack, int startX, int startY, int endX, int endY, AdvancementWidgetAccessor widgetAccessor, AdvancementProgressAccessor progressAccessor) {
        matrixStack.pushPose();
        RenderSystem.color4f(1.0F, 1.0F, 1.0F, 1.0F);
        RenderSystem.enableBlend();
        Minecraft.getInstance().getTextureManager().bind(PaginatedAdvancementScreen.WINDOW_TEXTURE);

        // corners
        blit(matrixStack, startX, startY, 0, 0, ELEMENT_WIDTH, TOP_ELEMENT_HEIGHT); // top left
        blit(matrixStack, endX - ELEMENT_WIDTH, startY, 237, 0, ELEMENT_WIDTH, TOP_ELEMENT_HEIGHT); // top right
        blit(matrixStack, startX, endY - BOTTOM_ELEMENT_HEIGHT, 0, 125, ELEMENT_WIDTH, BOTTOM_ELEMENT_HEIGHT); // bottom left
        blit(matrixStack, endX - ELEMENT_WIDTH, endY - BOTTOM_ELEMENT_HEIGHT, 237, 125, ELEMENT_WIDTH, BOTTOM_ELEMENT_HEIGHT); // bottom right
        ...
        while (middleHeight > 0) {
            ...
            blit(matrixStack, startX, currentY, 0, TOP_ELEMENT_HEIGHT, ELEMENT_WIDTH, currentDrawHeight);
            blit(matrixStack, endX - ELEMENT_WIDTH, currentY, 237, TOP_ELEMENT_HEIGHT, ELEMENT_WIDTH, currentDrawHeight);
        ...
        while (middleWidth > 0) {
            ...
            blit(matrixStack, currentX, startY, ELEMENT_WIDTH, 0, currentDrawWidth, TOP_ELEMENT_HEIGHT);
            blit(matrixStack, currentX, endY - BOTTOM_ELEMENT_HEIGHT, ELEMENT_WIDTH, 125, currentDrawWidth, BOTTOM_ELEMENT_HEIGHT);
        ...
        while (drawHeight > 0) {
            ...
            while (drawWidth > 0) {
                ...
                blit(matrixStack, drawStartX, drawStartY, 4, 4, currentWidth, currentHeight);
    ...
    public void addAdvancement(Advancement advancement) {
        if (advancement.getDisplay() != null) {
            AdvancementEntryGui advancementWidget = new PaginatedAdvancementWidget(this, this.client, advancement, advancement.getDisplay());
            this.addWidget(advancementWidget, advancement);
        }
    }

    private void addWidget(AdvancementEntryGui widget, Advancement advancement) {
    ...
    public void calculatePan() {
        for (AdvancementEntryGui widget : this.widgets.values()) {
    ...
    @Override
    @Nullable
    public AdvancementEntryGui getWidget(Advancement advancement) {
    ...
    public void copyHoveredAdvancementID() {
        if (this.hoveredWidget != null) {
            ...
            Minecraft.getInstance().keyboardHandler.setClipboard(awa.getAdvancement().getId().toString());
            Minecraft.getInstance().gui.setOverlayMessage(new TranslationTextComponent("text.paginated_advancements.copied_to_clipboard"), false);
        }
    }
}
