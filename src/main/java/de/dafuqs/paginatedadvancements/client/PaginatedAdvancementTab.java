...
public class PaginatedAdvancementTab extends AdvancementTab {
    ...
    public @NotNull AdvancementTabType getType() {
    ...
    public @NotNull DisplayInfo getDisplay() {
        return this.display;
    }
    ...
    public void render(GuiGraphics context, int startX, int startY, int endX, int endY) {
        ...
        ResourceLocation identifier = bg != null ? bg : TextureManager.INTENTIONAL_MISSING_TEXTURE;
    ...
    public void drawDebugInfo(GuiGraphics context, int startX, int startY, int endX, int endY) {
        if (this.hoveredWidget != null) {
            ...
            List<MutableComponent> requirements = getRequirements(startX, endX - 10, advancementWidgetAccessor.getAdvancement(), progress);
            ...
            if (PaginatedAdvancementsClient.CONFIG.ShowAdvancementIDInDebugTooltip) {
                Component idText = Component.literal("ID: " + advancementWidgetAccessor.getAdvancement().getId() + " ").append(Component.translatable("text.paginated_advancements.copy_to_clipboard"));
                context.drawString(this.client.font, idText, startX + 5, startY + 5, 0xFFFFFF, true);
            } else {
    ...
    protected void drawDebugFrame(GuiGraphics context, int startX, int startY, int endX, int endY) {
        ...
        // corners
        context.blit(PaginatedAdvancementScreen.WINDOW_TEXTURE, startX, startY, 0, 0, ELEMENT_WIDTH, TOP_ELEMENT_HEIGHT, 256, 256); // top left
        context.blit(PaginatedAdvancementScreen.WINDOW_TEXTURE, endX - ELEMENT_WIDTH, startY, 237, 0, ELEMENT_WIDTH, TOP_ELEMENT_HEIGHT, 256, 256); // top right
        context.blit(PaginatedAdvancementScreen.WINDOW_TEXTURE, startX, endY - BOTTOM_ELEMENT_HEIGHT, 0, 125, ELEMENT_WIDTH, BOTTOM_ELEMENT_HEIGHT, 256, 256); // bottom left
        context.blit(PaginatedAdvancementScreen.WINDOW_TEXTURE, endX - ELEMENT_WIDTH, endY - BOTTOM_ELEMENT_HEIGHT, 237, 125, ELEMENT_WIDTH, BOTTOM_ELEMENT_HEIGHT, 256, 256); // bottom right
        ...
        while (middleHeight > 0) {
            ...
            context.blit(PaginatedAdvancementScreen.WINDOW_TEXTURE, startX, currentY, 0, TOP_ELEMENT_HEIGHT, ELEMENT_WIDTH, currentDrawHeight, 256, 256);
            context.blit(PaginatedAdvancementScreen.WINDOW_TEXTURE, endX - ELEMENT_WIDTH, currentY, 237, TOP_ELEMENT_HEIGHT, ELEMENT_WIDTH, currentDrawHeight, 256, 256);
        ...
        while (middleWidth > 0) {
            ...
            context.blit(PaginatedAdvancementScreen.WINDOW_TEXTURE, currentX, startY, ELEMENT_WIDTH, 0, currentDrawWidth, TOP_ELEMENT_HEIGHT, 256, 256);
            context.blit(PaginatedAdvancementScreen.WINDOW_TEXTURE, currentX, endY - BOTTOM_ELEMENT_HEIGHT, ELEMENT_WIDTH, 125, currentDrawWidth, BOTTOM_ELEMENT_HEIGHT, 256, 256);
        ...
        while (drawHeight > 0) {
            ...
            while (drawWidth > 0) {
                ...
                context.blit(PaginatedAdvancementScreen.WINDOW_TEXTURE, drawStartX, drawStartY, 4, 4, currentWidth, currentHeight, 256, 256);
    ...
    protected void drawRequirementsWithOverflow(GuiGraphics context, int startX, int startY, int endX, int endY, List<MutableComponent> requirements, int lines) {
        for (int i = 0; i < lines; i++) {
            if (i == lines - 1) {
                context.drawString(this.client.font, Component.translatable("text.paginated_advancements.expand_debug"), startX, startY, 0x999999, false);
            } else {
                context.drawString(this.client.font, requirements.get(i), startX, startY, 0x00ff00, false);
            }
            startY += 10;
        }
    }
    ...
    protected void drawRequirements(GuiGraphics context, int startX, int startY, int endX, int endY, List<MutableComponent> requirements) {
        ...
        if (scrollAmount > 0) {
            context.drawString(this.client.font, Component.translatable("text.paginated_advancements.scroll_debug"), startX, startY, 0x999999, false);
            scrollAmount += 1;
            startY += 10;
        }
    ...
    public void addAdvancement(Advancement advancement) {
        ...
        if (display != null) {
            AdvancementWidget advancementWidget = new PaginatedAdvancementWidget(this, this.client, advancement, display);
    ...
    @Nullable
    @Override
    public AdvancementWidget getWidget(@NotNull Advancement advancement) {
        return this.widgets.get(advancement);
    }

    public @NotNull PaginatedAdvancementScreen getScreen() {
        return this.screen;
    }
