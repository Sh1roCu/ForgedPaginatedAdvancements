...
public class PaginatedAdvancementWidget extends AdvancementWidget {
    ...
    public PaginatedAdvancementWidget(AdvancementTab tab, Minecraft client, AdvancementNode placedAdvancement, DisplayInfo display) {
        ...
        int i = Math.max(accessor.getTitle().stream().mapToInt(client.font::width).max().orElse(0), 80);
        int j = this.getMaxProgressWidth();
        int l = 29 + i + j;

        if (this.frameWrapper != null) {
            this.description = Language.getInstance().getVisualOrder(accessor.invokeWrapDescription(ComponentUtils.mergeStyles(display.getDescription().copy(), frameWrapper.getTitleStyle()), l));
        } else {
    ...
    public void draw(@NotNull GuiGraphics context, int x, int y) {
        ...
        if (!accessor.getDisplay().isHidden() || accessor.getProgress() != null && accessor.getProgress().isDone()) {
            ...
            if (frameWrapper != null) {
                context.blitSprite(RenderType::guiTextured, frameWrapper.getTexture(advancementObtainedStatus, accessor.getDisplay().getType()), x + accessor.getX() + 3, y + accessor.getY(), 26, 26);
                ...
                context.blitSprite(RenderType::guiTextured, advancementObtainedStatus.frameSprite(accessor.getDisplay().getType()), x + accessor.getX() + 3, y + accessor.getY(), 26, 26);
    ...
    public void drawHover(@NotNull GuiGraphics context, int originX, int originY, float alpha, int x, int y) {
        ...
        if (!this.description.isEmpty()) {
            if (bl2) {
                context.blitSprite(RenderType::guiTextured, TITLE_BOX_TEXTURE, startX, l + 26 - n, accessor.getWidth(), n);
            } else {
                context.blitSprite(RenderType::guiTextured, TITLE_BOX_TEXTURE, startX, l, accessor.getWidth(), n);
            }
        }

        context.blitSprite(RenderType::guiTextured, advancementObtainedStatus.boxSprite(), 200, 26, 0, 0, startX, l, j, 26);
        context.blitSprite(RenderType::guiTextured, advancementObtainedStatus2.boxSprite(), 200, 26, 200 - k, 0, startX + j, l, k, 26);

        if (this.frameWrapper != null) {
            context.blitSprite(RenderType::guiTextured, this.frameWrapper.getTexture(advancementObtainedStatus3, accessor.getDisplay().getType()), originX + accessor.getX() + 3, originY + accessor.getY(), 26, 26);
        } else {
            context.blitSprite(RenderType::guiTextured, advancementObtainedStatus3.frameSprite(accessor.getDisplay().getType()), originX + accessor.getX() + 3, originY + accessor.getY(), 26, 26);
        }

        if (shouldRenderToTheLeft) {
            this.drawMultilineText(context, accessor.getTitle(), startX + 5, originY + accessor.getY() + 9, -1);
            if (string != null) {
                context.drawString(textRenderer, string, originX + accessor.getX() - i, originY + accessor.getY() + 9, -1);
            }
        } else {
            this.drawMultilineText(context, accessor.getTitle(), originX + accessor.getX() + 32, originY + accessor.getY() + 9, -1);
            if (string != null) {
