...
public class PaginatedAdvancementWidget extends AdvancementWidget {
    ...
    //private static final ResourceLocation TITLE_BOX_TEXTURE = ResourceLocation.tryParse("advancements/title_box");
    ...
    public PaginatedAdvancementWidget(AdvancementTab tab, Minecraft client, Advancement placedAdvancement, DisplayInfo display) {
        ...
        this.frameWrapper = AdvancementFrameDataLoader.get(accessor.getAdvancement().getId());

        int requirementCount = placedAdvancement.getRequirements().length;
    ...
    public void draw(@NotNull GuiGraphics context, int x, int y) {
        ...
        if (!accessor.getDisplay().isHidden() || accessor.getProgress() != null && accessor.getProgress().isDone()) {
            ...
            if (frameWrapper == null) {
                context.blit(VANILLA_WIDGETS_TEXTURE, x + accessor.getX() + 3, y + accessor.getY(), accessor.getDisplay().getFrame().getTexture(), 128 + advancementObtainedStatus.getIndex() * 26, 26, 26);
                context.renderItem(accessor.getDisplay().getIcon(), x + accessor.getX() + 8, y + accessor.getY() + 5);
            } else {
                context.blit(frameWrapper.getTextureSheet(), x + accessor.getX() + 3, y + accessor.getY(), frameWrapper.getTextureU(), frameWrapper.getTextureV() + advancementObtainedStatus.getIndex() * 26, 26, 26);
    ...
    public void drawHover(@NotNull GuiGraphics context, int originX, int originY, float alpha, int x, int y) {
        ...
        if (!description.isEmpty()) {
            if (bl2) {
                context.blitNineSliced(VANILLA_WIDGETS_TEXTURE, startX, l + 26 - n, accessor.getWidth(), n, 10, 200, 26, 0, 52);
            } else {
                context.blitNineSliced(VANILLA_WIDGETS_TEXTURE, startX, l, accessor.getWidth(), n, 10, 200, 26, 0, 52);
            }
        }

        context.blit(VANILLA_WIDGETS_TEXTURE, startX, l, 0, advancementObtainedStatus.getIndex() * 26, j, 26);
        context.blit(VANILLA_WIDGETS_TEXTURE, startX + j, l, 200 - k, advancementObtainedStatus2.getIndex() * 26, k, 26);
        if (this.frameWrapper == null) {
            context.blit(VANILLA_WIDGETS_TEXTURE, originX + accessor.getX() + 3, originY + accessor.getY(), accessor.getDisplay().getFrame().getTexture(), 128 + advancementObtainedStatus3.getIndex() * 26, 26, 26);
        } else {
            RenderSystem.setShaderTexture(0, this.frameWrapper.getTextureSheet());
            context.blit(VANILLA_WIDGETS_TEXTURE, originX + accessor.getX() + 3, originY + accessor.getY(), this.frameWrapper.getTextureU(), this.frameWrapper.getTextureV() + advancementObtainedStatus3.getIndex() * 26, 26, 26);
            RenderSystem.setShaderTexture(0, VANILLA_WIDGETS_TEXTURE);
        }
